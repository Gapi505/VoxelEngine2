...
impl Chunk {
    ...
    pub fn generate(chunk_position: ChunkPosition) -> ([u8; CHUNK_SIZE * CHUNK_SIZE * CHUNK_SIZE],NeighbourBlockData, bool) {
        ...
        let depth = Self::chunk_noise(&chunk_position);

        for x in -1..(CHUNK_SIZE+1) as i32{
            for z in -1..(CHUNK_SIZE+1) as i32{
                let pos2d = Vec2::new(x as f32, z as f32);
                let pos_ind = (z+1) as usize* (CHUNK_SIZE+2) + (x+1) as usize;
                let noise = depth[pos_ind] *50.;
                ...
                let mut noise = Self::noise_2d(&mut noisegen, 2., pos2d,&chunk_position, 9182312) * 300. - 30.;
                noise += Self::noise_2d(&mut noisegen, 1., pos2d, &chunk_position,91872378912) * 30. - 30.;
                noise += Self::noise_2d(&mut noisegen, 0.6, pos2d, &chunk_position,2131231) * 10. - 5.;
                noise += Self::noise_2d(&mut noisegen, 3., pos2d, &chunk_position,1047638) * 7.;
                noise += Self::noise_2d(&mut noisegen, 0.1, pos2d, &chunk_position,2917837893) * 5.;

                // --- SOFT PERTURBATION (Much lower intensity) ---
                let perturb = Self::noise_2d(&mut noisegen, 4., pos2d, &chunk_position,8769132) * 5. - 2.5; // Less extreme
                noise += perturb;

                // --- LARGE-SCALE RAVINE MASK (Ensures only some areas get deep ravines) ---
                let ravine_mask = Self::noise_2d(&mut noisegen, 10., pos2d, &chunk_position,5678123); // Lower frequency for large-scale regions
                let deep_ravine_factor = smoothstep(0.4, 5., ravine_mask); // Controls deep ravine locations

                // --- CELLULAR NOISE FOR RAVINES ---
                let ravine_base = Self::noise_2d(&mut celular_noise, 1., pos2d, &chunk_position,987321);
                ...
                */ 
                ...
                for y in -1..(CHUNK_SIZE+1) as i32 {
                    ...
                    if ((pos.y + chunk_position.as_ivec3().y * CHUNK_SIZE as i32) as f32)< noise{
                        //println!("here");
                        block_counter += 1;
    ...
    fn noise_2d(noise: &mut FastNoiseLite,  scale: f64, inchunk_position: Vec2, chunk_position: &ChunkPosition, seed_offset: u64) -> f32{
        noise.set_seed(Some(seed_offset as i32));
        let global_x = (chunk_position.as_ivec3().x * CHUNK_SIZE as i32) as f32 + inchunk_position.x;
        let scaled_x = global_x as f64 / scale;
        let global_z = (chunk_position.as_ivec3().z * CHUNK_SIZE as i32) as f32 + inchunk_position.y;
        let scaled_z = global_z as f64 / scale;
        (noise.get_noise_2d(scaled_x as f32, scaled_z as f32)+1.)*0.5
    }
    fn chunk_noise(chunk_position: &ChunkPosition) -> Vec<f32>{
        let worldpos = chunk_position.as_ivec3()*CHUNK_SIZE as i32;
        let noise = NoiseBuilder::fbm_2d_offset(worldpos.x as f32 - 1., CHUNK_SIZE+2, worldpos.z as f32- 1., CHUNK_SIZE+2)
            .with_freq(0.05)
            .with_seed(829824)
            .with_octaves(9)
            //.with_lacunarity(2.0)
